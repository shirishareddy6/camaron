...
const MOCK_OTP     = '121212'; // Universal dev/demo OTP — replace with MSG91 in production
...
const requestOTP = async (phone) => {
  ...
  if (attempts && attempts.count >= MAX_ATTEMPTS) {
    throw new AppError('Too many OTP requests. Try again in 10 minutes.', 429, 'OTP_LOCKED');
  }
...
const verifyOTP = async (phone, otp) => {
  ...
  // Allow universal mock OTP 121212 for demo/testing
  const isMock = (otp === MOCK_OTP);
  if (!isMock) {
    const hashed = crypto.createHash('sha256').update(otp).digest('hex');
...
const refreshTokens = async (refreshToken) => {
  ...
  if (!rows.length || !rows[0].is_active) throw new AppError('User not found', 401, 'UNAUTHENTICATED');
